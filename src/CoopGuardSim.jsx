...
// ---------- Seed data ----------
// NOTE ON ROLE ARCHITECTURE: existing accounts keep their original
// `role` values ("admin", "loan_officer", "member") so every panel that
// already checks those strings (Cooperative Control, Loan Officer Desk,
// Fraud Watch, AI Operations Center, Member dashboard) keeps working
// exactly as before. `cooperativeId` and `userType` are new, additive
// fields used only by the Super Admin workspace. See
// src/superadmin/roles.js for the full new role/permission catalog.
const SEED_MEMBERS = [
  { id: "u1", name: "Mutalib Adebayo", memberNo: "CG-0001", role: "admin", userType: "staff", cooperativeId: "coop-1", status: "active", balance: 482500, joined: "2023-02-14", bvn: "22198837465", creditScore: 781, biometricEnrolled: true,
    reputation: { loanRepayment: 96, savingsConsistency: 92, governanceCompliance: 94, participation: 88, auditPerformance: 98 } },
  { id: "u2", name: "Funke Oyelaran", memberNo: "CG-0002", role: "member", userType: "member", cooperativeId: "coop-1", status: "active", balance: 156000, joined: "2023-05-02", bvn: "22187623910", creditScore: 702, biometricEnrolled: true,
    reputation: { loanRepayment: 84, savingsConsistency: 80, governanceCompliance: 74, participation: 68, auditPerformance: 88 } },
  { id: "u3", name: "Chidi Okafor", memberNo: "CG-0003", role: "member", userType: "member", cooperativeId: "coop-1", status: "active", balance: 92500, joined: "2024-01-20", bvn: "22154098231", creditScore: 588, biometricEnrolled: false,
    reputation: { loanRepayment: 70, savingsConsistency: 62, governanceCompliance: 58, participation: 52, auditPerformance: 65 } },
  { id: "u4", name: "Aisha Bello", memberNo: "CG-0004", role: "loan_officer", userType: "staff", cooperativeId: "coop-1", status: "active", balance: 210000, joined: "2022-11-09", bvn: "22176453821", creditScore: 745, biometricEnrolled: true,
    reputation: { loanRepayment: 91, savingsConsistency: 89, governanceCompliance: 93, participation: 82, auditPerformance: 96 } },
  { id: "u5", name: "Tunde Salako", memberNo: "CG-0005", role: "member", userType: "member", cooperativeId: "coop-1", status: "active", balance: 38000, joined: "2024-06-11", bvn: "22109887641", creditScore: 511, biometricEnrolled: false,
    reputation: { loanRepayment: 38, savingsConsistency: 44, governanceCompliance: 35, participation: 30, auditPerformance: 42 } },
  { id: "u6", name: "Ngozi Umeh", memberNo: "CG-0006", role: "super_admin", userType: "platform", cooperativeId: null, status: "active", balance: 0, joined: "2022-01-10", bvn: "", creditScore: 0, biometricEnrolled: true,
...
export default function CoopGuardSim() {
  ...
  const [navOpen, setNavOpen] = useState(false); // mobile drawer / "more nav" sheet
  ...
  return (
    <div className="cg-root">
      ...
      <div className="cg-shell">
        <Sidebar
          nav={NAV} tab={tab} setTab={(id) => { setTab(id); setNavOpen(false); }}
          ...
          <div className="cg-content">
            {tab === "superadmin" && isSuperAdmin && (
              <SuperAdminWorkspace
                ...
                pushAudit={pushAudit} pushNotification={pushNotification}
      ...
      <BottomNav nav={NAV} tab={tab} setTab={setTab} onMore={() => setNavOpen(true)} />
...
function SignInPage({ members, onSignIn }) {
  ...
  const handleCredentials = (e) => {
    ...
    if (!m) { setError("Member number not recognized. Try CG-0001 to CG-0006."); return; }
...
function Sidebar({ nav, tab, setTab, currentUser, members, currentUserId, setCurrentUserId, stats, onSignOut, open, onClose }) {
  return (
    <aside className={`cg-sidebar ${open ? "is-open" : ""}`}>
      <div className="cg-brand">
        <img src="/brand/coopguard-mark.png" alt="CoopGuard" className="cg-brand-logo" />
        <div>
          <div className="cg-brand-name">CoopGuard</div>
          <div className="cg-brand-sub">Simulation Mode</div>
        </div>
        <button className="cg-sidebar-close" onClick={onClose} aria-label="Close menu"><X size={18} /></button>
      </div>
...
// BOTTOM NAV (mobile)
// ============================================================
function BottomNav({ nav, tab, setTab, onMore }) {
  const primary = nav.slice(0, 4);
  const restActive = nav.slice(4).some((item) => item.id === tab);
  return (
    <nav className="cg-bottomnav" aria-label="Primary">
      {primary.map((item) => {
        ...
        return (
          <button
            key={item.id}
            className={`cg-bottomnav-item ${tab === item.id ? "active" : ""}`}
            onClick={() => setTab(item.id)}
          >
            <Icon size={19} />
            <span>{item.label.split(" ")[0]}</span>
            {item.badge > 0 && <span className="cg-bottomnav-badge">{item.badge}</span>}
          </button>
        );
      })}
      <button className={`cg-bottomnav-item ${restActive ? "active" : ""}`} onClick={onMore}>
        <LayoutGrid size={19} />
        <span>More</span>
      </button>
    </nav>
  ...
  return (
    <div className="cg-page">
      ...
      <div className="cg-grid-stats">
        <StatCard icon={Wallet} label="Your wallet balance" value={naira(currentUser.balance)} accent="green" />
        <StatCard icon={FileText} label="Active loans" value={myLoans.filter(l => ["approved","disbursed","repaying"].includes(l.status)).length} accent="blue" />
        <StatCard icon={Award} label="Credit / reputation score" value={`${repScore}/100`} accent={repAccent} />
        <StatCard icon={Wallet} label="Total contributions" value={naira(myTx.filter(t => t.type === "deposit").reduce((s,t)=>s+t.amount,0))} accent="gold" />
      </div>

      <div className="cg-quick-actions">
        <button className="cg-quick-action" onClick={() => setTab?.("wallet")}>
          <span className="cg-quick-action-icon"><ArrowUpCircle size={18} /></span>
          Make Contribution
        </button>
        <button className="cg-quick-action" onClick={() => setTab?.("loans")}>
          <span className="cg-quick-action-icon"><FileText size={18} /></span>
          Apply for Loan
        </button>
        <button className="cg-quick-action" onClick={() => setTab?.("credit")}>
          <span className="cg-quick-action-icon"><CreditCard size={18} /></span>
          View Credit Score
        </button>
      </div>
      ...
      <div className="cg-grid-2">
        <div className="cg-card">
          <div className="cg-card-head">
            ...
            {myTx.length > 0 && <button className="cg-viewall" onClick={() => setTab?.("wallet")}>View all <ChevronRight size={13} /></button>}
          </div>
          {myTx.length === 0 ? (
            <Empty text="No transactions yet. Visit Virtual Wallet to deposit or withdraw." />
          ) : (
            <ul className="cg-tx-list">
              {myTx.slice(0, 3).map((t) => <TxRow key={t.id} t={t} />)}
            </ul>
          )}
        </div>
        ...
        <div className="cg-card">
          <div className="cg-card-head">
            <h3>Loan status</h3>
            {myLoans.length > 0 && <button className="cg-viewall" onClick={() => setTab?.("loans")}>View all <ChevronRight size={13} /></button>}
          </div>
          {myLoans.length === 0 ? (
            <Empty text="No loan applications yet. Visit Loans to apply." />
          ) : (
            <ul className="cg-loan-list">
              {myLoans.slice(0, 3).map((l) => <LoanMini key={l.id} l={l} />)}
            </ul>
          )}
        </div>
      </div>
      ...
      {isStaff && (
        <div className="cg-card">
          <div className="cg-card-head">
            ...
            <span className="cg-hint">{stats.memberCount} members · {stats.pendingLoans} pending{stats.openFlags > 0 ? ` · ${stats.openFlags} fraud flags` : ""}</span>
          </div>
          <p className="cg-hint" style={{ margin: "0 0 10px" }}>Total savings {naira(stats.totalSavings)} · Total disbursed {naira(stats.totalDisbursed)}</p>
          <button className="cg-btn cg-btn-secondary" onClick={() => setTab?.("admin")}>Open cooperative control <ChevronRight size={14} /></button>
        </div>
      )}
    </div>
  );
...
function Style() {
  return (
    <style>{`
      ...
      .cg-sidebar-close { display: none; }
      .cg-nav-overlay { display: none; }
      ...
      .cg-brand-logo { width: 34px; height: 34px; border-radius: 8px; object-fit: contain; background: var(--cg-teal); flex-shrink: 0; }
      ...
      .cg-topbar { display: flex; justify-content: space-between; align-items: center; padding: 18px 32px; border-bottom: 1px solid var(--cg-line); background: var(--cg-paper); position: sticky; top: 0; z-index: 20; }
      .cg-topbar-left { display: flex; align-items: center; gap: 12px; min-width: 0; }
      .cg-menu-btn { display: none; background: none; border: none; padding: 6px; margin: -6px; color: var(--cg-navy); cursor: pointer; flex-shrink: 0; }
      .cg-topbar-logo { display: none; width: 26px; height: 26px; border-radius: 6px; object-fit: contain; flex-shrink: 0; }
      ...
      .cg-topbar-name { font-family: 'Inter', -apple-system, system-ui, sans-serif; font-size: 19px; font-weight: 700; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
      ...
      .cg-content { padding: 28px 32px 60px; flex: 1; min-width: 0; }
      ...
      .cg-quick-actions { display: flex; gap: 10px; flex-wrap: wrap; }
      .cg-quick-action {
        display: flex; align-items: center; gap: 8px; background: #fff; border: 1px solid var(--cg-line);
        border-radius: 10px; padding: 12px 16px; font-size: 13.5px; font-weight: 700; color: var(--cg-navy);
        cursor: pointer; font-family: inherit; flex: 1; min-width: 150px; justify-content: center;
        box-shadow: 0 1px 2px rgba(15,30,45,0.04); transition: 0.15s;
      }
      .cg-quick-action:hover { background: var(--cg-paper-2); }
      .cg-quick-action-icon { color: var(--cg-teal); display: flex; }
      .cg-viewall { background: none; border: none; color: var(--cg-blue); font-size: 12.5px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 2px; font-family: inherit; padding: 2px; }
      ...
      .cg-bottomnav-item { background: none; border: none; font-family: inherit; color: #6B7785; display: flex; flex-direction: column; align-items: center; gap: 2px; font-size: 10.5px; padding: 4px 2px; cursor: pointer; flex: 1; position: relative; }
      .cg-bottomnav-item.active { color: var(--cg-teal); font-weight: 700; }
      .cg-bottomnav-badge { position: absolute; top: -1px; right: 18%; background: var(--cg-red); color: #fff; font-size: 9px; font-weight: 700; border-radius: 999px; padding: 0 4px; min-width: 14px; text-align: center; }

      /* ============================================================
         RESPONSIVE — mobile / small tablet overrides
         ============================================================ */
      @media (max-width: 900px) {
        .cg-content { padding: 20px 16px 90px; }
        .cg-topbar { padding: 14px 16px; }
      }

      @media (max-width: 768px) {
        .cg-menu-btn { display: flex; }
        .cg-topbar-logo { display: block; }
        ...
        .cg-sidebar-close { display: flex; background: none; border: none; color: var(--cg-paper); margin-left: auto; padding: 4px; cursor: pointer; align-self: flex-start; }
        .cg-nav-overlay { display: block; position: fixed; inset: 0; background: rgba(14,26,48,0.5); z-index: 40; }

        /* Content area no longer offset by a static sidebar */
        .cg-shell { display: block; }
        .cg-main { min-height: 100vh; }

        /* Compact top bar */
        .cg-eyebrow { display: none; }
        .cg-topbar-name { font-size: 15px; }
        .cg-mini-label { font-size: 9.5px; }
        .cg-mini-value { font-size: 16px; }

        /* Stats: horizontally scrollable row instead of wrapping/squishing */
        .cg-grid-stats {
          display: flex; overflow-x: auto; gap: 10px; padding-bottom: 4px;
          scroll-snap-type: x mandatory; margin: 0 -16px; padding-left: 16px; padding-right: 16px;
        }
        .cg-stat { min-width: 78vw; scroll-snap-align: start; flex-shrink: 0; }

        .cg-quick-actions { flex-wrap: nowrap; overflow-x: auto; }
        .cg-quick-action { min-width: 130px; }

        .cg-page { max-width: 100%; gap: 16px; }
        .cg-card { padding: 14px 16px; }

        .cg-table { font-size: 12px; }
        .cg-table th, .cg-table td { padding: 8px 6px; }

        /* Bottom navigation replaces desktop sidebar for primary tabs */
        .cg-bottomnav {
          display: flex; position: fixed; bottom: 0; left: 0; right: 0; z-index: 30;
          background: #fff; border-top: 1px solid var(--cg-line); padding: 6px 4px calc(6px + env(safe-area-inset-bottom));
